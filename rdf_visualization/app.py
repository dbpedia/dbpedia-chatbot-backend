...
def show_graph(graphId):
    ...
    net.force_atlas_2based(
        gravity=GRAVITY, spring_length=SPRING_LENGTH, central_gravity=CENTRAL_GRAVITY)
...
def html_page():
    ...
    sparql_database.setReturnFormat(XML)
    sparql_database.setMethod(POST)
    print("fnsfndskjfnsdkjfndskjfndskjf0")
    results = sparql_database.queryAndConvert()
    print("fnsfndskjfnsdkjfndskjfndskjf1")
    print(results)
    networkx_graph = rdflib_to_networkx_graph(results)
    net = Network(height=HEIGHT, width="100%")
    net.from_nx(networkx_graph)
...
if __name__ == '__main__':
    ...
    if os.environ.get("SSL_CERT") and os.environ.get("SSL_KEY"):
        app.run(host='0.0.0.0', port=port, ssl_context=(
            os.environ.get("SSL_CERT"), os.environ.get("SSL_KEY")))
    else:
